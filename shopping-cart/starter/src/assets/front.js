...
document.querySelector('.pay').addEventListener('click', (e) => {
    ...
    if (cart.length === 0) {
        alert('Your cart is empty. Please add items to your cart before paying.');
        document.querySelector('.received').value = '';
        document.querySelector('.received').focus();
        return;
    }
    
    // Get input cash received field value
    let amountInput = document.querySelector('.received').value.trim();
    
    // Validate input - check if it's empty
    if (amountInput === '') {
        alert('Please enter an amount for cash received.');
        document.querySelector('.received').focus();
        return;
    }
    
    // Validate input - check if it's a valid positive number
    // First, check if it starts with a minus sign (negative number)
    if (amountInput.startsWith('-')) {
        alert('Please enter a positive number for cash received.');
        document.querySelector('.received').value = '';
        document.querySelector('.received').focus();
        return;
    }
    ...
    if (isNaN(amount) || amount <= 0) {
        alert('Please enter a valid positive number for cash received.');
        document.querySelector('.received').value = '';
        document.querySelector('.received').focus();
        return;
    }
    ...
    // Else request additional funds
    if (cashReturn >= 0) {
        div.innerHTML = `<p>Cash Received: ${currencySymbol}${amount.toFixed(2)}</p>
        <p>Cash Returned: ${currencySymbol}${cashReturn.toFixed(2)}</p>
        <p>Thank you!</p>
        <p>********************************************************************************</p>`;
        // Clear the cash field after successful payment
        document.querySelector('.received').value = '';
        // Redraw cart and checkout to show empty cart
        drawCart();
        drawCheckout();
    } else {
        // reset cash field for next entry
